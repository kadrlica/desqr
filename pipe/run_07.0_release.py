...
SECTIONS = ['footprint','stargal','color','astrometry','depth','photometry','slr']
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--types', default=None, action='append')
    ...
    if 'depth' in sections:
        ...
        for t,snrs in types:
            if args.types and t not in args.types: continue
            print(f"  type: {t}")
            for snr in snrs:
    ...
    if 'astrometry' in sections:
        ...
        for t,bands in types:
            ...
            for b in bands:
                ...
                if t not in ['hpx_rms']: 
                    print(f"INFO: External astrometry runs one band: {b}.")
                    break
    ...
    if 'photometry' in sections:
        ...
        for t,bands in types:
            ...
            if t.startswith('gaia'):
                bands = ['griz']
                print(f"INFO: Gaia photometry uses all bands: {bands}")
            for b in bands:
                cmd = '%s %s -b %s --type %s'%(exe,args.configfile,b,t)
                cmd += ' --nproc %s'%args.nproc # number of parallel processes
                cmd += ' -v' if args.verbose else ''
    ...
    if 'slr' in sections:
        print("Calculting stellar locus regression...")
        exe = 'stellar_locus.py'
        
        types = ['best', 'wavg']
        #types += ['psf_mag', 'bdf_mag']

        for t in types:
            if args.types and t not in args.types: continue
            print(f"  type: {t}")

            cmd = '%s %s --type %s'%(exe, args.configfile, t)
            cmd += ' --nproc %s'%args.nproc # number of parallel processes
            cmd += ' -v' if args.verbose else ''
            
            logfile = os.path.join(logdir,'slr_%s.log'%(t))
            submit = 'csub -q %s -n %s -o %s %s'%(args.queue,args.njobs,logfile,cmd)
            if args.verbose: print(submit)
            subprocess.call(submit,shell=True)
